<template>
   <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
     ...
     <el-breadcrumb-item>发车信息删除</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card>
    <el-form ref="vedelFormRef" :model="queryInfo" :rules="rules" class="del_form"  label-width="100px">
...
export default{
  ...
  methods: {
    resetdelForm () {
      this.$refs.vedelFormRef.resetFields()
    },
    async basedel () {
      const confirmResult = await this.$confirm('确定删除发车报表吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).catch(err => err)
      if (confirmResult !== 'confirm') return this.$message.info('取消操作！')
      this.$refs.vedelFormRef.validate(async valid => {
        if (!valid) return
        const userinf = JSON.parse(window.sessionStorage.getItem('userinf'))
        this.queryInfo.userxm = userinf.userxm
        try {
          const {data: res} = await this.$http.post('vehicledel', this.queryInfo)
          if (res === 'success') {
